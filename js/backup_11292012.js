...
$( '#report-outage-page' ).live( 'pageshow',function(event){
	...
	if($form.find("input[name=account_number]").val().length > 0){
...
$form.find("input[name=phone_number]").bind("keyup change", function(event){
	...
	$form.find("input[name=phone_number]").bind("blur", function(event){
		if(stripAlpha($(this).val()).length == 10){
			$form.find("input[name=account_number]").attr("disabled", "disabled");
		}
		else{
			$form.find("input[name=account_number]").removeAttr("disabled");
	...
	//var validator = $currentPage.find("form#report-outage-form").validate({
	var validator = $form.validate({
		rules: {
	    	//account_number: {require_from_group: [1,".outage-locator-identifier"]},
	    	account_number: {require_from_group: [1,".outage-locator-identifier"], digits: true, maxlength: 10, minlength: 10},
	    	phone_number: {require_from_group: [1,".outage-locator-identifier"], phoneUS: true}
...
BGE.Form.setupContactForm = function($form){
	...
	var validator = $form.validate({
		...
		var count=0;
		if($pocDrop.val() == "Lighting Outages"){
		...
		else if($pocDrop.val() == "Vegetation/Tree Trimming"){
			 $('.altAddress').each(function() {
				 if(this.id && this.value!=null){
					   formData[count]={"FieldName":this.id,"Fieldvalue":this.value};
					   count++;
		...
		if(CustomerAddressEncrypt!=null){
			var formMetadata = { 	
	        	"FormTypeId":FormTypeID ,
	        	"FormName": FormName,
	        	"emailAddress":"sivaprasad.avvaru@constellation.com",
	        	"FormData":formData,
...
BGE.Form.resetContactForm = function($form){
	...
	//	$(this).val("");
		...
		if($(this).closest(".form-section-about-you").length == 0){
			$(this).val("");
		}
	});
...
$( '#fuzzy-account-con-page' ).live( 'pageinit',function(event){
 ...
 function getAddress() {
        ...
        $.ajax({
            url: '/_layouts/Bge.Mobile.Services/AnonymousService.asmx/GetCustomerDetails',
            type: 'POST',
			...
			ConCustomerAddresses = data.d.DisplayAddress;
                ...
                if(data.d.ReturnStatus == "ERROR"){
                	...
                	recognized=true;
                	if(data.d.CustomerAddressData.length > 1){
                    	 enteredPhoneNumber=$('#phone_number').val();
                	    ...
                	    // only one records found (start)
                		 ...
                		 $.each(ConCustomerData, function (idx) {
 	                          enteredAccountNumber =$('#account_number').val();
 	                          enteredPhoneNumber=$('#phone_number').val();
 	                          selectedAddress =this.DisplayAddress[0]; 
						    });
                		$.mobile.changePage( '../contact/form.html');
    ...
    function loadCaptchaTo(imageHolder) {
        ...
        $.ajax({
            url: '/_layouts/Bge.Mobile.Services/AnonymousService.asmx/GetCaptchaQuestion',
            type: 'POST',
            dataType: 'JSON',
            contentType: "application/json; charset=utf-8",
            success: function (data, textStatus, xhr) {
    ...
    function checkCaptcha() {
        ...
        $.ajax({
            url: '/_layouts/Bge.Mobile.Services/AnonymousService.asmx/CheckCaptcha',
            type: 'POST',
...
if(referenceNumber!=null){
	...
	$('#con_confirmation_number').text("hellllllllllllllllllll");
}
else{
...
function submitContactUsForm(formMetadata) {
	    ...
	    $.mobile.showPageLoadingMsg();
        $.ajax({
            url: '/_layouts/Bge.Mobile.Services/AnonymousService.asmx/SubmitContactUsForm',
            type: 'POST',
...
$( '#fuzzy-account-pao-page' ).live( 'pageinit',function(event){
	...
	$("#address-list").append('<tr><td><strong>Service Addr:</strong></td><td><strong>Est. Time of Restoral</strong></td></tr>').trigger('create');
 	$.each(CustomerDataOutage.OutageData, function (idx) {
 		$("#address-list").append('<tr><td><input type="radio" name="add" data-theme="b" id="add-'+idx+'" value="' + this.Address + '"  /><label for="add-'+idx+'">' + this.Address + '</label></td><td>'+this.ETRDescription+'</td></tr>').trigger('create');
    });
...
$( '#report-outage-page' ).live( 'pageinit',function(event){
	...
	loadCaptchaTo($('#rao-captcha'));
	$('#catptha-error').hide();
	$('#report-outage-form').submit(function() {
		$('#catptha-error').hide();
		getRAOAddress();
		return false;
	});
 ...
 function getRAOAddress() {
		...
		$.mobile.showPageLoadingMsg();
        $.ajax({
            url: '/_layouts/Bge.Mobile.Services/AnonymousService.asmx/GetCustomerDataOutage',
            type: 'POST',
            data: JSON.stringify(customerRequest),
            contentType: "application/json; charset=utf-8",
            dataType: 'JSON',
